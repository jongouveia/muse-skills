...
Muse-written entries that fail the gate land in `drafts/entries/` automatically during `prebuild`, so they cannot ship through `npm run build` or Vercel. Run `npm run gate:fix` to attempt the conservative em-dash auto-fix, or `npm run gate` to check entries in `src/content/entries/`.
